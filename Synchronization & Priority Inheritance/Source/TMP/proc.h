...
#define	PRLOCKED	'\011'		/* process is on lock queue*/
...
struct	pentry	{
	...
	int waitStart;
	int pinh ;					/* inherited priority of the process*/
	int	lockbitmask[NLOCKS];	/* lockBitmask[idx] > 0 means lock idx acquired, else lock idx is free */
	/*Note: lockbitmask stores the version# of the locks this process has acquired,
		lockBitmask[idx] < 0, lockBitmask[idx] = -(last used lock version)
	*/
	
	int lockid;					/* lock if process waiting	*/
	int lwaitret;
