...
/* emulate 386 "pushal" instruction */
	...
	*pushsp = pptr->pesp = (unsigned long)saddr;
	...
	// get a BS for vheap
	int bs_id;
	int get_bsm_ret = get_bsm(&bs_id);
	
	if(get_bsm_ret == SYSERR){
		kill(pid);
		restore(ps);
		return(SYSERR);
	}
	...
	proctab[pid].store = bs_id;                  /* backing store for vheap      */
    proctab[pid].vhpno = VPG_START;                  /* starting pageno for vheap    */
    proctab[pid].vhpnpages = hsize; 
	
	// one block of virtual memory
